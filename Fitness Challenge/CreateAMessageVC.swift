...
class CreateAMessageVC: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
   ...
   let kBannerAdUnitID = "ca-app-pub-3940256099942544/2934735716"
    ...
    override func viewDidLoad() {
        ...
        DataService.ds.REF_USERS.child(self.userKey).child("profile").observeSingleEvent(of: .value, with: {(snapshot) in
    ...
    deinit {
        ...
        if let refHandle = _refHandle {
            DataService.ds.REF_USER_CURRENT.child("messages").child(self.userKey).removeObserver(withHandle: refHandle)
        }
        //Receivers reference handle
        if let refHandle2 = _refHandle2 {
            self.ref.child(self.userKey).child("messages").child(currentUser).removeObserver(withHandle: refHandle2)
        }
    }
    ...
    func configureDatabase() {
        ...
        _refHandle2 = self.ref.child("users").child(self.userKey).child("messages").child(currentUser).observe(.childAdded, with: {[weak self] (snapshot) -> Void in
    ...
    func sendMessage(withData data: [String: String]) {
        ...
        self.ref.child("users").child(self.userKey).child("messages").child(currentUser).childByAutoId().setValue(mdata)
        self.ref.child("users").child(currentUser).child("messages").child(self.userKey).childByAutoId().setValue(mdata)
    }
