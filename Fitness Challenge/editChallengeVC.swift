...
class editChallengeVC: UIViewController, UITextFieldDelegate, UITextViewDelegate, UINavigationBarDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func submitChallenge(_ sender: Any) {
        ...
        if let title = titleTF.text, let description = descriptionTextView.text, let challengeTime = durationTF.text,let imgURL = imageLbl.text ,(title.characters.count > 0 && description.characters.count > 0 && challengeTime.characters.count > 0 && imgURL.characters.count > 0) {
            DataService.ds.REF_CHALLENGES.child(challengeKey).updateChildValues(["title": title ,"description": description,"challengeTime": challengeTime, "imageURL": imgURL, "time": Firebase.ServerValue.timestamp(), "createdBy": currentUser, "logoLocation": self.logoLocation])
            //Loads the logo to storage
            //loadLogoToStorage()
            
            let alertController1 = UIAlertController(title: "Success", message:"Challenge has been edited!", preferredStyle: .alert)
    ...
    //Fills challenge cells with prior information
    func fillChallenge() {
        ...
        DataService.ds.REF_CHALLENGES.child(challengeKey).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            self.descriptionTextView.text = snapshot.childSnapshot(forPath: "description").value! as! String
            self.durationTF.text = snapshot.childSnapshot(forPath: "challengeTime").value! as? String
            self.imageLbl.text = snapshot.childSnapshot(forPath: "imageURL").value! as? String
...
extension editChallengeVC: UIImagePickerControllerDelegate {
